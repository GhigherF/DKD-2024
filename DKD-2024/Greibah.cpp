...
namespace GRB
{
	Greibach greibach(NS('S'), TS('$'), 16,
		...
		Rule(NS('S'), GRB_ERROR_SERIES, 3,
			...
			Rule::Chain(6, TS('v'), TS('f'), TS('i'), NS('P'), NS('G'), NS('S')),
			Rule::Chain(4, TS('m'), TS('['), NS('K'), TS(']'))
		),

		Rule(NS('T'), GRB_ERROR_SERIES + 2, 2,					// тело функции
			Rule::Chain(5, TS('['), TS('e'), NS('V'), TS(';'), TS(']')),
			Rule::Chain(6, TS('['), NS('K'), TS('e'), NS('V'), TS(';'), TS(']'))
		),

		Rule(NS('G'), GRB_ERROR_SERIES + 3, 2,					// тело процедуры
			Rule::Chain(4, TS('['), TS('e'), TS(';'), TS(']')),
			Rule::Chain(5, TS('['), NS('K'), TS('e'), TS(';'), TS(']'))
		),

		Rule(NS('P'), GRB_ERROR_SERIES + 1, 2,					//параметры	
			Rule::Chain(3, TS('('), NS('E'), TS(')')),
			Rule::Chain(2, TS('('), TS(')'))
		),

		Rule(NS('E'), GRB_ERROR_SERIES + 4, 2,					//параметры						
			Rule::Chain(4, TS('t'), TS('i'), TS(','), NS('E')),
			Rule::Chain(2, TS('t'), TS('i'))
		),

		Rule(NS('F'), GRB_ERROR_SERIES + 5, 2,					// функция					
			Rule::Chain(3, TS('('), NS('N'), TS(')')),
			Rule::Chain(2, TS('('), TS(')'))
		),

		Rule(NS('N'), GRB_ERROR_SERIES + 6, 4,					// параметры	
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('l')),
		...
		Rule(NS('R'), GRB_ERROR_SERIES + 7, 5,					// цикл
		...
		Rule(NS('Y'), GRB_ERROR_SERIES + 8, 1,					// условие		
			Rule::Chain(3, TS('['), NS('X'), TS(']'))
		),

		Rule(NS('Z'), GRB_ERROR_SERIES + 9, 4,					// условие
		...
		Rule(NS('L'), GRB_ERROR_SERIES + 10, 4,					// операции
			Rule::Chain(1, TS('<')),
			Rule::Chain(1, TS('>')),
			Rule::Chain(1, TS('&')),
			Rule::Chain(1, TS('!'))
		...
		Rule(NS('A'), GRB_ERROR_SERIES + 11, 6,					//арифметика
			Rule::Chain(1, TS('+')),
			Rule::Chain(1, TS('-')),
			Rule::Chain(1, TS('*')),
			Rule::Chain(1, TS('/')),
		...
		Rule(NS('V'), GRB_ERROR_SERIES + 12, 3,					// всякое
			Rule::Chain(1, TS('l')),
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('b'))
		),

		Rule(NS('W'), GRB_ERROR_SERIES + 13, 8,					//выражении
			Rule::Chain(1, TS('i')),
			Rule::Chain(1, TS('l')),
		...
		Rule(NS('K'), GRB_ERROR_SERIES + 14, 14,
			Rule::Chain(7, TS('a'), TS('t'), TS('i'), TS('='), NS('V'), TS(';'), NS('K')),	// декларация + присваивание
			Rule::Chain(5, TS('a'), TS('t'), TS('i'), TS(';'), NS('K')),	// декларация
			...
			Rule::Chain(6, TS('a'), TS('t'), TS('i'), TS('='), NS('V'), TS(';')),	// декларация + присваивание
			Rule::Chain(4, TS('i'), TS('='), NS('W'), TS(';')),			// присваивание
			Rule::Chain(4, TS('a'), TS('t'), TS('i'), TS(';')),			// декларация
