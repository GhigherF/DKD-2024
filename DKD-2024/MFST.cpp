...
namespace MFST
{
	...
	Mfst::RC_STEP Mfst::step(const Log::LOG& log) //выполнить шаг автомата
	{
		...
		if (lenta_position < lenta_size)
		{
			if (ISNS(st.top())) //извлекаем последний элемент стека и проверяем нетерминал ли он
			{
				...
				if ((nrule = grebach.getRule(st.top(), rule)) >= 0)//смотрим, если такое правило есть идём дальше
				{
					...
					else
					{
						...
						//MFST_TRACE4_C("TNS_NORULECHAIN/NS_NORULE");//вывод
			...
			else { MFST_TRACE4(log, "TS_NOK/NS_NORULECHАIN"); /*MFST_TRACE4_C("TS_NOK/NS_NORULECHАIN");*/ rc = reststate(log) ? TS_NOK : NS_NORULECHAIN; }; //нет правил для цепочки
		}
		else {
			rc = LENTA_END; MFST_TRACE4(log, "LENTA_END"); //MFST_TRACE4_C("LENTA_END");
		};
		return rc;
	};
	...
	bool Mfst::savestate(const Log::LOG& log) //сохранить состояние автомата(для отката)
	{
		...
		//MFST_TRACE6_C("SAVESTATE:", storestate.size());
		return true;
	};
	...
	bool Mfst::start(const Log::LOG& log)
	{
		...
		{
		case LENTA_END: {
			MFST_TRACE4(log, "------>LENTA_END");// MFST_TRACE4_C("------>LENTA_END");
		}
					  *log.stream << "-------------------------------------------------------------------------------------" << std::endl;
					  ...
					  *log.stream << std::setw(4) << std::left << 0 << ":всего строк " << lenta_size << ", синтаксический анализ выполнен без ошибок " << std::endl;
					  ...
					  break;
		case NS_NORULE: {
			MFST_TRACE4(log, "------>NS_NORULE");// MFST_TRACE4_C("------>NS_NORULE");
		}
					  *log.stream << "-------------------------------------------------------------------------------------" << std::endl;
					  ...
					  *log.stream << getDiagnosis(2, buf) << std::endl;
					  ...
					  break;
		case NS_NORULECHAIN: {
			MFST_TRACE4(log, "------>NS_NORULECHAIN"); //MFST_TRACE4_C("------ > NS_NORULECHAIN");
		} break;
		case NS_ERROR: {
			MFST_TRACE4(log, "------>NS_ERROR"); //MFST_TRACE4_C("------ > NS_NORULECHAIN");
		}break;
		case SURPRISE: {
			MFST_TRACE4(log, "------>SURPRISE"); MFST_TRACE4_C("------ > SURPRISE"); break;
		};
					 return rc;
