...
namespace Lexer
{
	Graph graphs[N_GRAPHS] =
	{
		...
		{ LEX_ID_TYPE, FST::FST(GRAPH_SMALL) },
		{ LEX_ANNOUNCE, FST::FST(GRAPH_ANNOUNCE) },
		...
		{ LEX_LITERAL_BIN, FST::FST(GRAPH_B_LITERAL)},
		{ LEX_LITERAL_HEX, FST::FST(GRAPH_H_LITERAL)},
	...
	IT::IDDATATYPE getType(char* curword, char* idtype)
	{
		if (!strcmp(TYPE_VOID, idtype))
			return IT::IDDATATYPE::VOID; // процедуры
		if (!strcmp(TYPE_STRING, idtype))
			return IT::IDDATATYPE::STR;  // строковый ид
		if (!strcmp(TYPE_SMALL, idtype))
			return IT::IDDATATYPE::SML;	 // числовой  ид
		if (isdigit(*curword) || *curword == LEX_MINUS)
			return IT::IDDATATYPE::SML;				// числовой литерал
	...
	IT::STDFNC getStandFunction(char* id)
	{
		...
		if (!strcmp(STON, id))
			return IT::STDFNC::F_STON;
		if (!strcmp(DATE, id))
			return IT::STDFNC::F_DATE;
		...
		bool& lex_ok)							// флаг ошибки(по ссылке)
	{
		...
		else // идентификатор
		{
			...
			if (isFunc)
			{
				...
				{
				case IT::STDFNC::F_STON:
				{
					itentry->idtype = IT::IDTYPE::S;
					itentry->iddatatype = STON_TYPE;
					itentry->value.params.count = STON_PARAMS_CNT;
					itentry->value.params.types = new IT::IDDATATYPE[STON_PARAMS_CNT];
					for (int k = 0; k < STON_PARAMS_CNT; k++)
						itentry->value.params.types[k] = IT::STON_PARAMS[k];
					break;
				}
				case IT::STDFNC::F_DATE:
				{
					itentry->idtype = IT::IDTYPE::S;
					itentry->iddatatype = STON_TYPE;
					itentry->value.params.count = DATE_PARAMS_CNT;
					break;
				}
		...
		if (i > 1 && itentry->idtype == IT::IDTYPE::V && tables.lextable.table[i - 2].lexema != LEX_ANNOUNCE)
		{
			// в объявлении отсутствует ключевое слово announce
				...
				if (FST::execute(fst))
				{
					...
					case LEX_SEPARATORS:
					{
						...
						case LEX_BRACELET:		// конец области видимости
						{
							...
							if (*in.words[i + 1].word == LEX_ID_TYPE || *in.words[i + 1].word == LEX_VOID || *in.words[i + 1].word == LEX_MAIN || *in.words[i + 1].word == LEX_FUNCTION)
							{
								if (!scopes.empty())
									scopes.pop();
							}
					...
					case LEX_LITERAL_BIN:
					{
						int value = DecimicalNotation(curword, 2);
						...
						else // повторный идентификатор (уже есть)
						{
							...
							if (i > 0 && tables.lextable.table[i - 1].lexema == LEX_ANNOUNCE || tables.lextable.table[i].lexema == LEX_ANNOUNCE
					...
					{
						int value = DecimicalNotation(curword,16);
						if (value < NUM_MINSIZE || value > NUM_MAXSIZE)
							throw ERROR_THROW(205);
					...
					case LEX_LITERAL:
					{
						...
						if (isLiteral(curword))
							strcpy_s(id, curword); // литерал заменяется своим значением
						if (curword[0] == 't'&& curword[1] == 'r'&& curword[2] == 'u'&& curword[3] == 'e')
						{
							strcpy_s(id,"1");
						}
						if (curword[0] == 'f' && curword[1] == 'a' && curword[2] == 'l' && curword[3] == 's'&&curword[4]=='e')
						{
							strcpy_s(id, "0");
						}
