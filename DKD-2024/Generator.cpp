...
namespace Gener
{
	...
	string genCallFuncCode(Lexer::LEX& tables, Log::LOG& log, int i)
	{
		...
		std::vector<std::string> assemblerKeywords = {
	"mov", "add", "sub", "mul", "div", "imul", "idiv", "inc", "dec",
	"cmp", "test", "and", "or", "xor", "not", "shl", "shr", "sal",
	"sar", "rol", "ror", "push", "pop", "lea", "xchg", "movsx",
	"movzx", "movs", "stos", "jmp", "call", "ret", "je", "jne",
	"jg", "jge", "jl", "jle", "ja", "jae", "jb", "jbe", "jp", "jnp",
	"loop", "stc", "clc", "cmc", "lahf", "sahf", "pushf", "popf",
	"nop", "hlt", "int", "iret", "in", "out", "cpuid", "fld",
	"fst", "fstp", "fadd", "fsub", "fmul", "fdiv", "fsqrt", "fcom",
	"fucom", "movd", "movq", "paddb", "paddw", "paddd", "psubb",
	"psubw", "psubd", "pmuludq", "pmaddwd", "andps", "orps", "xorps",
	"addps", "mulps", "divps", "subps", "cli", "sti", "lgdt",
	"sgdt", "lidt", "sidt"
		};
		for (auto& tmp : assemblerKeywords)
		{
			bool flag = true;
			for (int i = 0; i < tmp.length(); i++)
			{
				if (e.id[i] != tmp[i]) flag = false;
			}if (flag) throw ERROR_THROW(69);
		}
		stack <IT::Entry> temp;
	...
	string genFunctionCode(Lexer::LEX& tables, int i, string funcname, int pcount)
	{
		...
		while (LEXEMA(j) != LEX_RIGHTTHESIS) // пока параметры не кончатс€
		{
			if (LEXEMA(j) == LEX_ID) // параметр
				str = str + string(ITENTRY(j).id) + (ITENTRY(j).iddatatype == IT::IDDATATYPE::SML ? " : sdword, " : " : dword, ");
			j++;
		}
		int f = str.rfind(',');
		if (f > 0)
	...
	vector <string> startFillVector(Lexer::LEX& tables)
	{
		...
		for (int i = 0; i < tables.idtable.size; i++)// const, data
		{
			...
			if (tables.idtable.table[i].idtype == IT::IDTYPE::L)	// литерал - в .const
			{
				switch (e.iddatatype)
				{
				case IT::IDDATATYPE::SML:  str = str + " sdword " + itoS(e.value.vint);  break;
			...
			else if (tables.idtable.table[i].idtype == IT::IDTYPE::V)// переменна€ - в .data
			{
				switch (e.iddatatype)
				{
				case IT::IDDATATYPE::SML: str = str + " sdword 0";  break;
	...
	void CodeGeneration(Lexer::LEX& tables, Parm::PARM& parm, Log::LOG& log)
	{
		...
		ofstream ofile("C:/DKD-2024/DKD-2024/Generation/Generation/Gen.asm");
