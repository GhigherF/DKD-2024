	...
	void writeError(std::ostream* stream, Error::ERROR e)
	{
		if (stream == NULL)
		{
			if (e.position.col == -1 || e.position.line == -1)
				...
				std::cout << std::endl << "Ошибка N" << e.id << ": " << '\n' << e.message <<
		...
		{
			if (e.position.col == -1 || e.position.line == -1) {
				*stream << std::endl << "Ошибка N" << e.id << ": " << std::endl << e.message << std::endl;
				std::cout << std::endl << "Ошибка N" << e.id << ": " << std::endl << e.message << std::endl;

			}
			else if (e.position.col == NULL) {
				*stream << std::endl << "Ошибка N" << e.id << ": " << std::endl << e.message
					<< " Строка: " << e.position.line << std::endl;
				std::cout << std::endl << "Ошибка N" << e.id << ": " << std::endl << e.message << std::endl
					<< " Строка: " << e.position.line << std::endl;
			}
